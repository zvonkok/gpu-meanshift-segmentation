...
%  \pgfplotsset{every axis y label/.style={at={(0,1)}, left}}
  ...
  \begin{tikzpicture}
      ...
      ymin=0.3, ymax=1.7,
      xlabel=Image Size,
      ylabel=Normalized Ratio (size/time),
 			ytick={0,0.5,1,1.5},
			legend style={at={(0.03,0.95)}, anchor=north west}]
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[
			...
			% ymin=0,
			ymax=7000,
			xlabel=Image size, ylabel={Runtime [s]},
      ...
      \addplot[color=blue1,mark=*] table[x=PIX,y=1GPU] from \tableA;
			...
			\addlegendentry{Runtime \slcsmallcaps{1 GPU}}
			\addlegendentry{Runtime \slcsmallcaps{2 GPUs}}
...
% 	\pgfplotsset{every axis y label/.style={at={(0,1)}, left}}
  ...
  \begin{tikzpicture}
		\begin{axis}[
			...
			\addlegendentry{Speedup 1 \protect\gls{GPU}}
			\addlegendentry{Speedup 2 \protect\glspl{GPUs}}
