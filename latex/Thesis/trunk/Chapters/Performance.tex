...
the increase {\color{red}{TODO wenn die differenz gleich ansteigt und siehe wikipedia}}
ratio of the two variables is constant.
\begin{figure}[ht]
  \centering
...
be seen the ratio for the \gls{CPU} and the \gls{GPU} is increasing
similar to the speedup because of the before mentioned ramp-up phase until it
reaches 1 as in the case of the \gls{CPU} ratio. If the image size is doubled
the run-time is doubled (image size $\propto$ run-time) which means the
algorithm scales linearly. {\color{red}{TODO paragraph ueberpruefen}}
...
%      xmin=0, xmax=2796, 
    ...
    %  ymajorgrids 
 			...
 			\addplot[color=plot1!80!black,fill=plot1!40!white] table[x=PIX,y=Speedup] from \tableA;
 			\addplot table[x=PIX,y=CPU_Speedup] from \tableB;
...
ratio clock/time is depicted as well. 
...
%	      enlarge x limits=0.1,
 ...
 \end{tikzpicture}%
 \label{fig:clock_single_256}%
 \caption{Image 256x256 varying the core and mem clock}
\end{figure}

scales as well over the frequency is computational bound. restructuring
could help
...
%\fpAdd{\timecpu}{0,0}{1076056}
%\fpAdd{\timegpu}{0,0}{2734.836}

%\fpDiv{\speedup}{\timegold}{\timecuda}

FINAL SPEEDUP  CPU 

Equipped $\openg$ asda af
