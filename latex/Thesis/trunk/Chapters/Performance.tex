...
ratio \textbf{clock/time} is depicted as well. 
...
It's visible that the run-time is decreasing proportionally with the increase 
of the core-clock. This means that the algorithm is heavily computational bound.
...
\autoref{cha:future_work} for enhancements that could be made to accelerate
the search. 
